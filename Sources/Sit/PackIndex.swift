...
public struct PackIndexV2: Sendable {
  ...
  public func offset(for sha20: [UInt8]) -> UInt32? {
    ...
    while lo < hi {
      ...
      if c == 0 {
        ...
        guard off & 0x8000_0000 == 0 else {
          /// MSB set → the real offset lives in the 64-bit offset table (pack index v2
          /// extension), which is not yet supported.
          return nil
        }
        return off
      }
  ...
  public static func parse(bytes: [UInt8]) throws -> PackIndexV2 {
    ...
    for i in 0..<n {
      ...
      let crc = readBigEndianUInt32(b, crcBase + 4 * i)
      let off = readBigEndianUInt32(b, offBase + 4 * i)
